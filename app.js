...
function serviceFields(service) {
  ...
  return [
    ...
    ['teacherCostPerClassA', 'Pago por clase · Docente A', 'number', service?.teacherCostPerClassA ?? service?.teacherCostPerClass ?? 0, false],
    ['teacherCostPerClassB', 'Pago por clase · Docente B', 'number', service?.teacherCostPerClassB ?? service?.teacherCostPerClassA ?? service?.teacherCostPerClass ?? 0, false],
    ['teacherRatesHelp', 'Usa estas dos tarifas según la experiencia del docente. En cada escenario podrás elegir si aplica A o B.', 'help', '', false],
...
function scenarioItemFields(item) {
  ...
  return [
    ...
    ['teacherCategory', 'Tipo de docente', 'select', item?.teacherCategory || 'A', false, [{ value: 'A', label: 'Docente A' }, { value: 'B', label: 'Docente B' }]],
    ...
    ['customTeacherCostPerClass', 'Pago docente excepcional (reemplaza A/B)', 'number', item?.customTeacherCostPerClass ?? '', false],
...
function scenarioServiceSummary(service) {
  ...
  const rows = [
    ...
    ['Pago Docente A', formatCurrency(service.teacherCostPerClassA ?? service.teacherCostPerClass ?? 0)],
    ['Pago Docente B', formatCurrency(service.teacherCostPerClassB ?? service.teacherCostPerClassA ?? service.teacherCostPerClass ?? 0)],
...
function normalizeServiceForm(v) {
  ...
  return {
    ...
    teacherCostPerClassA: toNumber(v.teacherCostPerClassA), teacherCostPerClassB: toNumber(v.teacherCostPerClassB), teacherCostPerClass: toNumber(v.teacherCostPerClassA), materialsCostPerStudent: toNumber(v.materialsCostPerStudent), materialsCostPerClass: toNumber(v.materialsCostPerClass), variableCostPerSubscriber: toNumber(v.variableCostPerSubscriber),
...
function normalizeScenarioItemForm(v) {
  return {
    ...
    teacherCategory: v.teacherCategory === 'B' ? 'B' : 'A',
    autoGenerated: false,
    lockedManualValues: true,
